...
resource "aws_api_gateway_resource" "login_clientes" {
  ...
  path_part   = "login-clientes"
}

resource "aws_api_gateway_method" "post" {
  rest_api_id   = data.aws_api_gateway_rest_api.existing_api.id
  resource_id   = aws_api_gateway_resource.login_clientes.id
  http_method   = "POST"
  authorization = "NONE"
}
...
resource "aws_lambda_permission" "apigw" {
  ...
  source_arn = "${data.aws_api_gateway_rest_api.existing_api.execution_arn}/*/${aws_api_gateway_method.post.http_method}${aws_api_gateway_resource.login_clientes.path}"
}
...
resource "aws_api_gateway_deployment" "deploy" {
  ...
  triggers = {
    redeployment = sha1(jsonencode([
      aws_api_gateway_resource.login_clientes.id,
